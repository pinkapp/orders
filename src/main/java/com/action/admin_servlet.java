...
public class admin_servlet extends HttpServlet
{
	...
	{
        String type=req.getParameter("type");//取得参数
      
		if(type.endsWith("adminMana"))//这个类型是以adminmana结尾的
		{
			adminMana(req, res);
		}
	...
	{
		List adminList=new ArrayList();//创建一个数组
		String sql="select * from t_admin";
		Object[] params={};//定义object数组
		DB mydb=new DB();
		try
		{
	...
	public void adminAdd(HttpServletRequest req,HttpServletResponse res)
	{
		...
		Object[] params={userName,userPw};//数据库操作参数
		DB mydb=new DB();
		mydb.doPstm(sql, params);
		mydb.closed();
		...
		dispatch(targetURL, req, res);//路径跳转
